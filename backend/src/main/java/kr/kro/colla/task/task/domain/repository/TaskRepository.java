...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    @Query("select new kr.kro.colla.task.task.domain.repository.dto.TaskCountByStatus(t.taskStatus.name, count(t)) from Task t where t.project = :project group by t.taskStatus")
    List<TaskCountByStatus> groupByTaskStatus(@Param("project") Project project);

    @Query("select new kr.kro.colla.task.task.domain.repository.dto.TaskCountByStatus(t.taskStatus.name, count(t), u.name) from Task t left join fetch User u on u.id = t.managerId where t.project = :project group by t.taskStatus, t.managerId")
